...
const mutations = {
  ...
  [types.CLICK_TAB](stateContext: Lulumi.Store.State, payload: any): void {
    ...
    const safeIndex = tabIndex >= 0 && tabIndex < tabs.length ? tabIndex : 0;
    ...
    const fallback = tabs[safeIndex];
    const fallbackIndex = stateContext.tabs.findIndex(tab => tab.id === fallback.id);
    if (fallbackIndex !== -1) {
      Vue.set(stateContext.tabs[fallbackIndex], 'highlighted', true);
      Vue.set(stateContext.tabs[fallbackIndex], 'active', true);
    }
  },
  // tab handlers
